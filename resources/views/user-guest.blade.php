        ...
        <form class="px-8 pt-6 pb-8 mb-4" action="/kunjungan-langsung" method="POST" enctype="multipart/form-data">
          @csrf
            <div class="flex items-center justify-center mb-4">
                <img class="mx-auto size-1/3" src="{{ asset("images/logotni.png") }}" alt="">
              </div>
              <div class="flex items-center justify-center mb-4">
          ...
          <div class="mb-3">
            <label class="input input-bordered flex items-center gap-2 py-2" for="nama">
                <input type="text" name="nama" id="nama" class="grow" placeholder="Nama" autofocus readonly value="{{ auth()->user()->nama }}"/>
          ...
          <div class="mb-3">
            <label class="input input-bordered flex items-center gap-2 py-2 @error('telp') input-error @enderror" for="telp">
              ...
              <input type="tel" name="tel" class="w-7" placeholder="+62" readonly/>
              <input type="number" name="telp" id="telp" class="grow" placeholder="Telp" readonly value="{{ auth()->user()->telp }}"/>
          ...
          <div class="mb-3">
            <label class="input input-bordered flex items-center gap-2 py-2 @error('nik') input-error @enderror" for="nik">
                <input type="number" name="nik" id="nik" class="grow" placeholder="NIK" readonly value="{{ auth()->user()->nik }}"/>
          ...
          {{-- <div class="mb-3">
            <label class="form-control w-full max-w-xl @error('selfie') input-error @enderror">
                ...
                <input type="file" name="selfie" id="selfie" accept="image/*" capture="user" class="file-input file-input-bordered w-full max-w-xl" onchange="previewImage(event)" value="{{ asset('storage/'. $user->img_path) }}"/>
            </label>
            @error('selfie')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div> --}}
        {{-- <div id="preview-container" class="mb-3" style="">
            <img id="preview-image" src="{{ asset('storage/'. auth()->user()->img_path) }}" alt="Preview" class="w-full max-w-xl" />
        </div>            --}}
          <div class="flex items-center justify-center mb-4">
